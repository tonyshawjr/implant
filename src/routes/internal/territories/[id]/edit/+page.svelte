<script lang="ts">
  ...
  // Initialize from existing territory data
  let selectedAreas = $state<SelectedArea[]>([]);
  let demographics = $state<Demographics | null>(null);
  let territoryName = $state(data.territory.name);
  let monthlyBasePrice = $state(data.territory.monthlyBasePrice || 1500);

  // Delete confirmation
  let showDeleteConfirm = $state(false);
  let isDeleting = $state(false);

  // Initialize demographics from existing data
  $effect(() => {
    if (data.territory) {
      // Build initial demographics from territory data
      if (data.territory.population) {
        demographics = {
          population: data.territory.population || 0,
          households: data.territory.households || 0,
          medianIncome: data.territory.medianIncome || 0,
          medianAge: data.territory.medianAge || 0,
          population65Plus: data.territory.population65Plus || 0,
          population65PlusPercent: data.territory.population65Pct || 0,
          medianHomeValue: data.territory.medianHomeValue || 0,
          veteransCount: data.territory.veteransCount || 0,
          ownerOccupiedPercent: data.territory.ownerOccupiedPct || 0
        };
      }

      // Build initial selected area based on boundary type
      if (data.territory.boundaryType && selectedAreas.length === 0) {
        const initialArea: SelectedArea = {
          id: `${data.territory.boundaryType}-${data.territory.id}`,
          type: data.territory.boundaryType as 'metro' | 'county' | 'city' | 'zipcode',
          name: data.territory.name,
          displayName: data.territory.name,
          fips: data.territory.msaCode || data.territory.countyFips || data.territory.placeFips || undefined,
          state: data.territory.state,
          zipCodes: data.territory.zipCodes.map(z => ({
            zipCode: z.zipCode,
            city: z.city,
            state: data.territory.state,
            lat: data.territory.centerLat,
            lng: data.territory.centerLng
          })),
        ...
        // Set the add mode based on boundary type
        if (data.territory.boundaryType === 'metro' || data.territory.boundaryType === 'county' ||
            data.territory.boundaryType === 'city' || data.territory.boundaryType === 'zipcode') {
          addMode = data.territory.boundaryType;
        }
      }
    }
  });
  ...
  let priceAutoCalculated = $state(false);
  ...
  onMount(async () => {
    if (browser) {
      ...
      // Center on territory location
      const centerLat = data.territory.centerLat || 39.8283;
      const centerLng = data.territory.centerLng || -98.5795;

      map = L.map(mapContainer).setView([centerLat, centerLng], 8);
      L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
        attribution: '&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors'
      }).addTo(map);

      // Load boundary for existing territory
      await loadExistingBoundary();
    }
    return () => { if (map) map.remove(); };
  });

  // Load the boundary for the existing territory
  async function loadExistingBoundary() {
    if (!data.territory.boundaryType || data.territory.boundaryType === 'custom') {
      // For custom or unknown, just show a circle
      updateMapMarkers();
      return;
    }

    const bt = data.territory.boundaryType;
    let geoid = '';

    if (bt === 'metro' && data.territory.msaCode) {
      geoid = data.territory.msaCode;
    } else if (bt === 'county' && data.territory.countyFips) {
      const stateFips = STATE_FIPS[data.territory.state] || '';
      geoid = `${stateFips}${data.territory.countyFips}`;
    } else if (bt === 'city' && data.territory.placeFips) {
      const stateFips = STATE_FIPS[data.territory.state] || '';
      geoid = `${stateFips}${data.territory.placeFips}`;
    } else if (bt === 'zipcode' && data.territory.zipCodes.length > 0) {
      geoid = data.territory.zipCodes[0].zipCode;
    }

    if (geoid) {
      const boundary = await fetchBoundary(bt, geoid);
      if (boundary && selectedAreas.length > 0) {
        selectedAreas[0].boundary = boundary;
        updateMapMarkers();
      }
    } else {
      updateMapMarkers();
  ...
  $effect(() => {
    if (map && L && selectedAreas.length > 0) {
      updateMapMarkers();
    }
  });
  ...
  function updateMapMarkers() {
    ...
    selectedAreas.forEach(area => {
      ...
      if (area.boundary) {
        ...
        circle.bindPopup(`<strong>${area.displayName}</strong><br>${getAreaTypeLabel(area.type)}`);
        mapLayers.push(circle);
        allBounds.push(circle.getBounds());
      }
    });
  ...
  // Census API calls and other helper functions (same as create page)
  ...
  // Add functions for metro, county, city, zip (simplified versions)
...
<svelte:head>
  <title>Edit Territory - {data.territory.name} - SqueezMedia Operations</title>
...
<div class="page-header">
  <div class="page-header-left">
    ...
    <h1 class="page-title">Edit Territory</h1>
    <p class="page-subtitle">Modify {data.territory.name} - Update locations, demographics, and pricing.</p>
  </div>
...
<div class="builder-container">
  ...
  <div class="builder-sidebar">
    ...
    <div class="card">
      ...
      <div class="card-body">
        ...
        <div class="add-mode-tabs">
          <button class="mode-tab" class:active={addMode === 'zipcode'} onclick={() => { addMode = 'zipcode'; handleModeChange(); }}>Zip Codes</button>
          <button class="mode-tab" class:active={addMode === 'city'} onclick={() => { addMode = 'city'; handleModeChange(); }}>Cities</button>
          <button class="mode-tab" class:active={addMode === 'county'} onclick={() => { addMode = 'county'; handleModeChange(); }}>Counties</button>
          <button class="mode-tab" class:active={addMode === 'metro'} onclick={() => { addMode = 'metro'; handleModeChange(); }}>Metros</button>
        </div>
        ...
        {#if addMode === 'zipcode'}
          <div class="form-group">
            ...
            <div class="input-with-button">
              <input type="text" class="form-input" placeholder="e.g., 28401" bind:value={zipCodeInput} onkeydown={(e) => e.key === 'Enter' && addZipCode()} maxlength="5" autocomplete="off" />
              <button class="btn btn-primary" onclick={addZipCode} disabled={isLoading || !zipCodeInput.trim()}>
                {#if isLoading}<span class="spinner-sm"></span>{:else}Add{/if}
              </button>
            </div>
          </div>
        {:else}
          <div class="form-group">
            ...
            <div class="searchable-select">
              <input type="text" class="form-input" placeholder="Type to search states..." bind:value={stateSearch} onfocus={() => showStateDropdown = true} onblur={() => setTimeout(() => showStateDropdown = false, 200)} autocomplete="off" />
              ...
              {#if showStateDropdown && filteredStates.length > 0}
                <div class="dropdown-list">
                  {#each filteredStates.slice(0, 10) as state}
                    <button type="button" class="dropdown-item" class:selected={selectedState === state.code} onmousedown={() => { selectedState = state.code; stateSearch = ''; showStateDropdown = false; handleStateChange(); }}>{state.name}</button>
                  {/each}
                  {#if filteredStates.length > 10}
                    <div class="dropdown-hint">{filteredStates.length - 10} more - keep typing</div>
                  {/if}
                </div>
              {/if}
            </div>
          ...
          {#if selectedState}
            <div class="form-group">
              <label class="form-label">Select {addMode === 'metro' ? 'Metro Area' : addMode === 'county' ? 'County' : 'City'}</label>
              {#if isLoading}
                <div class="loading-inline"><span class="spinner-sm"></span><span>Loading...</span></div>
              {:else}
                <div class="input-with-button">
                  <div class="searchable-select flex-1">
                    <input type="text" class="form-input" placeholder="Type to search..." bind:value={optionSearch} onfocus={() => showOptionDropdown = true} onblur={() => setTimeout(() => showOptionDropdown = false, 200)} autocomplete="off" />
                    {#if selectedOption && !showOptionDropdown}
                      <div class="selected-value">
                        {#if addMode === 'metro'}{metroOptions.find(o => o.fips === selectedOption)?.name}{:else if addMode === 'county'}{countyOptions.find(o => o.fips === selectedOption)?.name}{:else}{cityOptions.find(o => o.fips === selectedOption)?.name}{/if}
                    ...
                    {#if showOptionDropdown && filteredOptions.length > 0}
                      <div class="dropdown-list">
                        {#each filteredOptions.slice(0, 15) as opt}
                          <button type="button" class="dropdown-item" class:selected={selectedOption === opt.fips} onmousedown={() => { selectedOption = opt.fips; optionSearch = ''; showOptionDropdown = false; }}>{opt.name}</button>
                        {/each}
                      </div>
                    {/if}
                  </div>
                  <button class="btn btn-primary" onclick={() => { if (addMode === 'metro') addMetroArea(); else if (addMode === 'county') addCounty(); else if (addMode === 'city') addCity(); }} disabled={isLoading || !selectedOption}>Add</button>
                </div>
              {/if}
            </div>
          {/if}
    ...
    <div class="card">
      ...
      <div class="card-body">
        <div class="form-group">
          ...
          <input type="text" class="form-input" placeholder="e.g., Greater Wilmington" bind:value={territoryName} autocomplete="off" />
        </div>
        ...
        <div class="form-group">
          ...
          <div class="input-with-prefix">
            ...
            <input type="number" class="form-input" bind:value={monthlyBasePrice} oninput={handlePriceChange} min="0" step="100" autocomplete="off" />
          </div>
          {#if demographics}
    ...
    <!-- Action Buttons -->
    <form method="POST" action="?/updateTerritory" use:enhance={() => {
      isUpdating = true;
      return async ({ result }) => {
        isUpdating = false;
        if (result.type === 'redirect') {
          ...
          errorMessage = result.data?.error || 'Failed to update territory';
        }
      };
    }}>
      ...
      <input type="hidden" name="state" value={selectedAreas[0]?.state || data.territory.state} />
      <input type="hidden" name="primaryCity" value={selectedAreas[0]?.name || allZipCodes[0]?.city || data.territory.city} />
      ...
      <input type="hidden" name="centerLat" value={selectedAreas[0]?.boundary?.properties?.CENTLAT || selectedAreas[0]?.centerLat || data.territory.centerLat} />
      <input type="hidden" name="centerLng" value={selectedAreas[0]?.boundary?.properties?.CENTLON || selectedAreas[0]?.centerLng || data.territory.centerLng} />
      ...
      <button type="submit" class="btn btn-primary btn-lg btn-full" disabled={!canUpdate || isUpdating}>
        {#if isUpdating}
  ...
  <div class="builder-main">
    ...
    <div class="card demographics-card">
      <div class="card-header">
        ...
        {#if isLoadingDemographics}<span class="spinner-sm"></span>{/if}
      </div>
      <div class="card-body">
        {#if demographics}
          <div class="demographics-grid">
            <div class="demo-item">
              <div class="demo-icon"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg></div>
              <div class="demo-content"><div class="demo-value">{formatNumber(demographics.population)}</div><div class="demo-label">Population</div></div>
            </div>
            <div class="demo-item">
              <div class="demo-icon"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg></div>
              <div class="demo-content"><div class="demo-value">{formatNumber(demographics.households)}</div><div class="demo-label">Households</div></div>
            </div>
            <div class="demo-item">
              <div class="demo-icon"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg></div>
              <div class="demo-content"><div class="demo-value">{formatCurrency(demographics.medianIncome)}</div><div class="demo-label">Median Income</div></div>
            </div>
            <div class="demo-item">
              <div class="demo-icon"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg></div>
              <div class="demo-content"><div class="demo-value">{demographics.medianAge}</div><div class="demo-label">Median Age</div></div>
            </div>
            <div class="demo-item highlight">
              <div class="demo-icon"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg></div>
              <div class="demo-content"><div class="demo-value">{formatNumber(demographics.population65Plus)} <span class="demo-percent">({demographics.population65PlusPercent.toFixed(1)}%)</span></div><div class="demo-label">Population 65+</div></div>
            </div>
            <div class="demo-item">
              <div class="demo-icon"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><path d="M9 22V12h6v10"/></svg></div>
              <div class="demo-content"><div class="demo-value">{formatCurrency(demographics.medianHomeValue)}</div><div class="demo-label">Median Home Value</div></div>
            </div>
          </div>
        {:else}
...
{#if showDeleteConfirm}
  ...
  <div class="modal modal-sm">
    <div class="modal-header">
      ...
      <button class="close-btn" onclick={() => showDeleteConfirm = false}>
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
          <line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/>
        </svg>
      </button>
    </div>
    <form method="POST" action="?/deleteTerritory" use:enhance={() => {
      isDeleting = true;
      return async ({ result }) => {
        isDeleting = false;
        if (result.type === 'redirect') {
          ...
          errorMessage = result.data?.error || 'Failed to delete territory';
      ...
      <div class="modal-body">
        <div class="delete-warning">
          ...
          <p>Are you sure you want to delete <strong>{data.territory.name}</strong>?</p>
      ...
      <div class="modal-footer">
        ...
        <button type="submit" class="btn btn-danger" disabled={isDeleting}>
          {#if isDeleting}Deleting...{:else}Delete Territory{/if}
        </button>
      </div>
    </form>
  </div>
